...
def calcular_troco(valor_produto, valor_pago, qtd_atualizados):
    ...
    #calcular troco se for menor ou igual ele vai colocar no troco
        ...
        for chave, valor in dinheiro.items():
            if valor[0] > troco:
                ...
                if troco_din > troco:
                    ...
                    i += 1
                    continue
                else: # se menor ou igual a zero e n vai dar o troco
                    if valor[1] <= 0:
...
while True:
    ...
    try:# so serve para erro de index
        if codigo == maquina[codigo][0]:
            if maquina[codigo][3] > 0:# quantidade
                print(f'O valor da(o) {maquina[codigo][1]} é R${maquina[codigo][2]}')# valor original do produto
